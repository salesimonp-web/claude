...
class SentimentAnalyzer:
    ...
    def get_perplexity_analysis(self, asset: str) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"Perplexity [{asset}]: {analysis[:200]}...")
            ...
            logger.error(f"Perplexity error for {asset}: {e}")
            return None
    ...
    def get_twitter_sentiment(self, asset: str) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                result = response.json()
                analysis = result['choices'][0]['message']['content']
                logger.info(f"Grok [{asset}]: {analysis[:200]}...")

                # Detect useless responses
                if self._is_useless_response(analysis):
                    logger.info(f"Grok [{asset}]: useless response (no Twitter data), skipping")
                    return None
            ...
            logger.error(f"Grok error for {asset}: {e}")
            return None
    ...
    def get_combined_bias(self, asset: str) -> Dict:
        ...
        logger.info(f"AI bias for {asset}: {bias} (score: {score:.2f}, source: Perplexity)")
